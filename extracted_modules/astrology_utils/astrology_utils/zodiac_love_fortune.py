...
# 同一パッケージから相対インポート
try:
    from .astro_system import AstroCalculator, GeminiInterpreter
except ImportError:
    # フォールバック: クラスが利用できない場合
    AstroCalculator = None
    GeminiInterpreter = None
    print("⚠️ AstroCalculator, GeminiInterpreter が利用できません。基本機能のみ提供します。")

class ZodiacLoveFortune:
    """星座別恋愛運システム"""
    
    def __init__(self):
        if AstroCalculator is not None:
            self.calculator = AstroCalculator()
        else:
            self.calculator = None
            
        if GeminiInterpreter is not None:
