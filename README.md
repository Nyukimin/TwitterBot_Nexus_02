...
### 🔒 **6. セキュリティ・プライバシー & プロセス管理**
- **X API不使用**: Selenium活用によるコスト効率・API制限回避
- **ローカル処理**: 全データはローカル処理・外部送信なし
- **プロファイル制ログイン**: Chromeプロファイルによる安全なログイン状態管理
- **自動プロセス管理**: 特定プロファイルのみのChromeプロセス終了機能
- **競合回避**: user-data-dir競合エラーの自動解決
...
### 3) ログイン支援（プロファイル制・自動プロセス管理）
**プロファイル制ログインシステム**でユーザー名プレフィルと手動ログインを補助します。
**特定プロファイルのみのChromeプロセス終了機能**により、user-data-dir競合エラーを自動解決します。
...
**機能**:
- **自動プロセス管理**: 該当プロファイルを使用中のChromeプロセスのみを自動終了
- **競合回避**: user-data-dir競合エラーを事前に防止
- **複数アカウント対応**: 他のプロファイルに影響を与えずに安全実行
- **永続ログイン**: 一度ログインすれば次回は自動的にログイン状態を維持
...
例: Maya19960330 アカウントのログインプロファイル作成
```bash
python -m reply_bot.login_assist --config .\config\accounts_Maya19960330.yaml --accounts Maya19960330
```

### **コマンドの説明**
- `--config <設定ファイルパス>`: 使用する accounts.yaml ファイルのパスを指定
- `--accounts <アカウントID>`: ログインプロファイルを作成したいアカウントの id を指定

### **🆕 自動プロセス管理機能**
1. **プロファイル特定のプロセス終了**: `psutil`ライブラリを使用して該当プロファイルを使用中のChromeプロセスのみを自動終了
2. **競合エラー防止**: user-data-dir競合エラーを事前に自動解決
3. **他アカウントへの影響なし**: 他のプロファイルを使用中のChromeセッションは継続
4. **安全な並行使用**: 複数アカウントの同時使用が可能

### **実行フロー**
1. **自動プロセスクリーンアップ**: 該当プロファイルのChromeプロセスを自動検出・終了
2. **Chrome起動**: 指定されたアカウントのプロファイルでChromeブラウザが起動
3. **ユーザー名自動入力**: ログイン画面でユーザー名が自動入力
4. **手動ログイン**: 初回のみ、パスワード・2FA認証を手動で完了
5. **プロファイル保存**: ログイン状態がプロファイルに永続保存

### **注意点**
- 初回のみ手動でTwitterにログインが必要
- ログイン完了後、Enterキーを押してプロセス完了
- 次回からは自動的にログイン状態が維持
- 以前の `add_user_preferences.py` は廃止（accounts.yaml の `policies.per_target` で設定管理）
