# TwitterBot_Nexus_02 プロジェクト概要

## 🎯 プロジェクトの目的

**AI技術を活用した高機能Twitterボットシステム** - 占星術、画像生成、テキスト処理、ブラウザ自動化を統合し、人間らしい自然な投稿を24時間自動で実現するプラットフォーム
...
- **ツイート情報抽出**: タイムラインからツイート情報を自動抽出
- **リプライ分析**: リプライツイートの構造解析
- **ユーザー情報取得**: ユーザーID、表示名、プロフィール情報の収集
- **タイムスタンプ処理**: ツイート投稿時刻の正確な取得・変換
- **データエクスポート**: 収集データのCSV/JSON形式でのエクスポート

```python
from reply_bot.extract_and_export_tweets import extract_tweet_info

# BeautifulSoupでツイート要素から情報を抽出
tweet_data = extract_tweet_info(tweet_article)
# 結果: {
#   'reply_id': '1234567890',
#   'content': 'ツイート本文',
#   'replier_id': 'username',
...
- **複数アカウントの同時管理**: 複数のTwitterアカウントを一元管理
- **スケジュール投稿**: 時間・曜日指定での自動投稿
- **人間的な動作パターン**: ランダム間隔で自然な投稿を実現
...
- **TwitterBot本体**: [`reply_bot/`](../reply_bot/) - メインボット機能
- **設定管理**: [`config/`](../config/) - アカウント・スケジュール設定
- **ログ管理**: [`logs/`](../logs/) - 実行ログ・エラーログ
...
このプロジェクトの詳細については、以下のドキュメントを参照してください：

- **[ドキュメント索引](00_DOCS_INDEX.md)** - 全ドキュメントの一覧と使い方
- **[コーディング開発ガイド](01_コーディング開発ガイド.md)** - 開発ルール・バッチ設定
- **[基本仕様と作成ガイド](04_基本仕様と作成ガイド.md)** - プロジェクト仕様詳細
- **[完全実装仕様書](08_完全実装仕様書.md)** - 詳細な構築ガイド
